...
export default function PatientRegistrationPage() {
  ...
  useEffect(() => {
    ...
    patientService.getAllPatients()
      .then((livePatients) => {
        console.log('[Patients Page Live DB Patients Received]', livePatients);
        if (livePatients && livePatients.length > 0) {
          setDbPatients(livePatients);
        }
      })
  ...
  // Fetch Front Desk Inbox (SP: getfrontdeskinboxdata) whenever the From/To date
 
  useEffect(() => {
    if (!fromDate || !toDate) return;

    patientService.getFrontDeskInbox({
      fromdate: formatDate(fromDate), // yyyy-mm-dd -> DD/MM/YYYY
      ...
      .catch((err) => {
        console.warn('Front desk inbox fetch warning:', err);
      });
  }, [fromDate, toDate]);
  ...
  const filteredRoster = (patients || [])
    ...
    .filter((p) => {
      ...
      // NOTE: date-range filtering now happens on the server (getfrontdeskinboxdata
      // SP receives fromdate + todate directly), so `patients` in the store already
      // only contains rows for the selected range. We don't re-filter by date here
      // because the inbox API's `registrationdate` is derived from a TIME value
      // (paymentdate), not a full date, and comparing that against fromDate/toDate
      // would incorrectly wipe out the roster.
      const isWithinDate = true;
      const isMatchingSearch =
        (p.fullname || '').toLowerCase().includes(rosterSearchTerm.toLowerCase()) ||
        (p.mobileno || '').includes(rosterSearchTerm) ||
        (p.gsspatid || '').toString().includes(rosterSearchTerm) ||
